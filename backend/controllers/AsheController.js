...
const Ashe = require('../config/models/AsheInventory');
const Asheduplicate = require('../config/models/Asheinventories_duplicates');
...
const sequelizeAshe = new Sequelize(dbsequelizeAshe);
const ProjectEmail = ProjectEmailModel(sequelize, Sequelize);

const storeDataAshe = async (req, res) => {
  try {
    const { dbName, modelName } = req.params;
    //const data = req.body.input_value;
    const data = req.body;
    ...
    const Model = getModel(sequelizeInstance, modelName);
       ...
       const expectedColumns = [
        'coding_date','claim_id','patient_name','mri_number','admit_date','discharge_date','insurance','location','user_name','actual_user','emp_id'
    ];
    ...
    if (duplicateClaimNumbers.length > 0) {
      const tableName = 'Asheinventories_duplicates'; 
...
async function sendEmailNotification(res, request) {
  ...
  try {
    const result = await ProjectEmail.findOne({
      where: { client_name: 'Asheinventories' },
    });
...
function getModel(sequelizeInstance, modelName) {
  switch (modelName) {
    case 'AsheInventory':
      return Ashe(sequelizeInstance);
    case 'Asheinventories_duplicates':   
      return Asheduplicate(sequelizeInstance);
      case 'ProjectEmail':   
      return ProjectEmail;
    default:
