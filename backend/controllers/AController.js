...
const AClientClaimsDetailsModel = require('../config/models/A_client_claims_details');
...
const sequelizeA = new Sequelize(dbConfigA);
...
async function saveDuplicateClaimData(dbName, duplicateTableName, duplicateClaimNumbers, originalData) {
  const sequelizeInstance = getSequelizeInstance(dbName);
  const DuplicateClaimsModel = getModel(sequelizeInstance, duplicateTableName);
  ...
  try {
    ...
    for (let i = 0; i < originalData.length; i++) {
      ...
      if (duplicateClaimNumbers.includes(item.claim_id) && !existingClaimIdsSet.has(item.claim_id)) {
        ...
        console.log(`Claim ${item.claim_id} inserted into ${duplicateTableName}`);
      } else {
        console.log(`Claim ${item.claim_id} already exists in ${duplicateTableName}, not inserted`);
      }
    }

    console.log(`Duplicate claim data saved in ${duplicateTableName}`);
  } catch (error) {
    console.error(`Error saving duplicate claim data for ${duplicateTableName}:`, error);
    throw error;
  }
}
...
function getModel(sequelizeInstance, modelName) {
  switch (modelName) {
    case 'A_client_claims_details':
      return AClientClaimsDetailsModel(sequelizeInstance);
