...
const PSC = require('../config/models/AIG_psc_inventory');
const PSCduplicate = require('../config/models/AIG_psc_inventory_duplicates');
...
const sequelizeAIGpsc = new Sequelize(dbsequelizeAIGpsc);
const ProjectEmail = ProjectEmailModel(sequelize, Sequelize);

const storeDataAIGpsc = async (req, res) => {
  try {
    ...
    const Model = getModel(sequelizeInstance, modelName);
       ...
       const expectedColumns = [
        'ticket_number','doctor','insurance_carrier','insurance_group','patient_id','patient_name','dos','doe','department','facility','company','visit_status','visit_desc','cpt','units','modifier','diagnoses','coder_cpt','coder_icd','coder_comments','emp_id'   
    ];
    ...
    if (duplicateClaimNumbers.length > 0) {
      const tableName = 'AIG_psc_inventory_duplicates'; 
...
function getSequelizeInstance(dbName) {
  if (dbName === 'AIG_PSC') {
    return sequelizeAIGpsc;
  }
}

function getModel(sequelizeInstance, modelName) {
  switch (modelName) {
    case 'AIG_psc_inventory':
      return PSC(sequelizeInstance);
    case 'AIG_psc_inventory_duplicates':   
      return PSCduplicate(sequelizeInstance);
      case 'ProjectEmail':   
      return ProjectEmail;
    default:
