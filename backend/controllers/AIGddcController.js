...
const Aig = require('../config/models/AigDdcInventory');
const Aigduplicate = require('../config/models/AigDdcInventoryDuplicates');
...
const sequelizeAIGddc = new Sequelize(dbsequelizeAIGddc);
const ProjectEmail = ProjectEmailModel(sequelize, Sequelize);

const storeDataAIGddc = async (req, res) => {
  try {
    ...
    if (duplicateClaimNumbers.length > 0) {
      const tableName = 'AigDdcInventoryDuplicates'; 
...
function getSequelizeInstance(dbName) {
  if (dbName === 'AIG_DDC') {
    return sequelizeAIGddc;
  }
}

function getModel(sequelizeInstance, modelName) {
  switch (modelName) {
    case 'AigDdcInventory':
      return Aig(sequelizeInstance);
    case 'AigDdcInventoryDuplicates':   
      return Aigduplicate(sequelizeInstance);
      case 'ProjectEmail':   
      return ProjectEmail;
    default:
